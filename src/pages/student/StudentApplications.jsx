...
export function useStudentActivity() {
  return useBrowserDraft("talentbridge-student-activity-v1", {
    savedIds: [],
    applications: [],
  });
}
  ...
  return (
    <main className="student-activity">
      <header className="sa-header">
        <Link to="/student/dashboard" className="sa-brand">
          TalentBridge
        </Link>
      ...
      <p className="sa-notice">
        Demo opportunities · Applications stay in this browser and are not
        sent to companies.
      </p>
...
function StorageStatus({ storage }) {
  return (
    <>
      <p className="sa-muted" role="status">{storage.status}</p>
    ...
    : "← Back to opportunities"}
      ...
      {applicationPage && storage.ready && !application && (
        <section className="sa-card">
          ...
          <p>Share your profile, introduce yourself, or include a link to your work.</p>
<form onSubmit={apply}>
  ...
  />
      ...
      {applicationPage && storage.ready && application && (
        <section className="sa-card">
          ...
          <p>You have applied for {opportunity.title} at {opportunity.company}.</p>
          <p className="sa-muted">
            This application has not been sent to the company.
            Check the browser saving status below.
          </p>
          <div className="sa-actions">
            <Link className="sa-primary" to="/student/applications">
    ...
    <PageLayout>
      ...
      {storage.ready && activity.applications.length === 0 && (
        <section className="sa-card sa-empty">
          ...
          <p>Explore opportunities and apply for one that matches your skills.</p>
  ...
  {application.profileAttachment ? (
    <section className="sa-attached-profile">
      ...
      {application.profileAttachment.profile.availability && (
        <>
          <h4>Availability</h4>
          <p>{application.profileAttachment.profile.availability}</p>
        </>
      )}
      ...
      {application.profileAttachment.projects.length > 0 && (
        <>
          <h4>Projects</h4>
          {application.profileAttachment.projects.map((project, index) => (
            <div key={index}>
              <strong>{project.title}</strong>
              <p className="sa-introduction">{project.description}</p>
            </div>
          ))}
        </>
      )}
    ...
    <p className="sa-muted">No profile was attached to this application.</p>
  )}
